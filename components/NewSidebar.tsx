...
export default function NewSidebar({ open, setOpen }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={collapsible}
        className={`rounded-full bg-primary_100 xl:p-[10px] p-2 focus:outline-none ${
      ...
      <aside data-collapse={collapse} className="sidebar">
        <div
          className={`flex flex-col justify-start items-start ${
            ...
            open && "mt-16"
          } xl:gap-4 lg:gap-3 md:gap-2 gap-1 xl:px-[46px] lg:px-9 md:px-6 px-2`}
        >
          <h1
            className={`font-poppins text-[#333] xl:text-[45px] lg:text-3xl text-2xl font-bold ${
          ...
          <p
            className={`xl:text-lg lg:text-base text-sm font-medium text-[#b9bbbd] ${
              collapse && "text-center"
            }`}
          >
            Admin dashboard
          ...
          {sidebarRoutes.map((route) => (
            ...
            >
              ...
              <Link
                ...
                href={route.path}
                className={`flex items-center justify-start xl:gap-6 lg:gap-4 gap-3 rounded-lg w-full xl:px-5 lg:px-[15px] px-[10px] ${
                  collapse && "ms-6 mb-3"
                } group ${
                  currentRoute.includes(route.path)
